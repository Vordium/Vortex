  ...
  async function fetchDexSwap() {
    ...
    try {
      const allowance = await axiosInstance.get(
        `/api/proxy/swap/v5.2/1/approve/allowance?tokenAddress=${tokenOne.address}&walletAddress=${address}`,
        { headers }
      );

      if (allowance.data.allowance === "0") {
        const approve = await axiosInstance.get(
          `/api/proxy/swap/v5.2/1/approve/allowance?tokenAddress=${tokenOne.address}`,
          { headers }
        );
      ...
      const tx = await axiosInstance.get(
        `/api/proxy/swap/v5.2/1/swap?fromTokenAddress=${tokenOne.address}&toTokenAddress=${tokenTwo.address}&amount=${tokenOneAmount.padEnd(
